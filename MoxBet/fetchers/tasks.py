...
async def process_day(fixtures_data, all_odds, sport, live=False):
    ...
    for nf in fixtures_data:
        ...
        # Current time as dynamic timestamp
        ...
        # --- Skip timestamp check for finished games ---
        ...
        # --- Finished games handling ---
        if status in FINISHED_STATUSES:
            expiry = 60 * 60 * 24 * 7  # keep finished games 7 days
            payload = {
                "match_id": fixture_id,
                "sport": sport,
                "country": country,
            ...
            try:
                ...
                # Clean up old live/match keys
                await redis_client.delete(cache_key)
                await redis_client.srem(f"live:{sport.lower()}", cache_key)

                print(f"[CACHED] {sport} finished game {fixture_id} ({status}) → moved to finished:*")
            except Exception as e:
                print(f"Redis error caching finished match {fixture_id}: {e}")
            continue  # skip rest since finished games don’t need odds
    ...
    # Save list of live IDs at the end
